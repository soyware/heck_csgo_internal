...
namespace Log
{
	...
	void Init()
	{
		LOG_HECK = RegisterLoggingChannel("heck", nullptr, LCF_CONSOLE_ONLY, LS_MESSAGE, Color::White());
	}
	...
	// those are displayed in notify
	template<typename ... Args>
	void PrefixedMsg(const char* pMessageFormat, Args ... args)
	{
		char prefixMsg[MAX_LOGGING_MESSAGE_LENGTH] = "[heck] ";
		strcat_s(prefixMsg, sizeof(prefixMsg), pMessageFormat);

		Msg(prefixMsg, args...);
	}

	template<typename ... Args>
	void PrefixedMsg(Color spewColor, const char* pMessageFormat, Args ... args)
	{
		char prefixMsg[MAX_LOGGING_MESSAGE_LENGTH] = "[heck] ";
		strcat_s(prefixMsg, sizeof(prefixMsg), pMessageFormat);

		Msg(spewColor, prefixMsg, args...);
	}
}