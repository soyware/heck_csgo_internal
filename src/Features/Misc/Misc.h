	...
	namespace Misc
	{
		...
		*/
		...
		void ShowImpacts(int userId, const vec3& pos)
		{
			...
			static vec3 zeroVec;
			Color clr(Settings::Visuals::ShowImpacts::Color.GetInt());
			I::DebugOverlay->AddBoxOverlay(pos, mins, maxs, zeroVec, clr.r, clr.g, clr.b, clr.a, Settings::Visuals::ShowImpacts::Duration.GetFloat());
		}
		...
		void DeathSound(int userId, int attacker)
		{
			if (!(*G::pGameRules) || (*G::pGameRules)->IsWarmupPeriod())
				return;
			...
			int playerIndex = I::EngineClient->GetLocalPlayer();
			if (playerIndex != I::EngineClient->GetPlayerForUserID(userId))
				return;

			C_CSPlayer* player = static_cast<C_CSPlayer*>(I::EntityList->GetClientEntity(playerIndex));
			if (!player)
				return;

			if (Settings::Misc::DeathSound::OnlyTeamkill.GetInt())
			{
				int killerIndex = I::EngineClient->GetPlayerForUserID(attacker);
				if (killerIndex == playerIndex)
					return;

				C_CSPlayer* killer = static_cast<C_CSPlayer*>(I::EntityList->GetClientEntity(killerIndex));
				if (!killer || (killer->GetTeam() != player->GetTeam()))
					return;
			}
