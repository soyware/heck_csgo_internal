	...
	namespace Moves
	{
		namespace Aim
		{
			namespace Backtrack
			{
				...
				float FindSimTime(C_CSPlayer* player, CUserCmd* cmd)
				{
					...
					for (int i = 0; i < G::Globals->maxClients; ++i)
					{
						...
						for (int j = 0; j < ticksStored; ++j)
						{
							...
							if (tr.DidHit())
							{
								int hitchance = GetHitchance(player, cmd, weapon, entity, (bestHitchance ? bestHitchance + 1 : 100));
				...
				void Run(C_CSPlayer* player, CUserCmd* cmd)
				{
					...
					float foundSimTime = FindSimTime(player, cmd);
					if (foundSimTime)
