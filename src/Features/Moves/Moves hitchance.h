...
namespace Features
{
	namespace Moves
	{
		...
		namespace Aim
		{
			...
			int GetHitchance(C_CSPlayer* player, CUserCmd* cmd, C_WeaponCSBaseGun* weapon, C_BaseEntity* target, int limit = 100)
			{
				if (limit <= 0)
					return 0;
				...
				for (int i = 0; i < rayCount; ++i)
				{
					...
					vec3 spread;
					GetSpread(weapon, cmd->buttons, spread);
					...
					hitchance = raysHit * 100 / rayCount;
		...
		namespace Aim
		{
			...
			void Triggerbot(C_CSPlayer* player, CUserCmd* cmd)
			{
				...
				for (int i = 1; i <= G::Globals->maxClients; ++i)
				{
					...
					int needed = Settings::Moves::Aim::Triggerbot::Hitchance.GetInt();
					if (GetHitchance(player, cmd, weapon, tr.pEnt, needed) < needed)
						continue;
